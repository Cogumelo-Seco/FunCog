    ...
    switch (type) {
        ...
        case 'started':
			...
			for (let i in state.arrowsInfo) {
				state.arrowsInfo[i].shadowBlur = 13
				state.arrowsInfo[i].noteShadowBlur = 13
				state.arrowsInfoOpponent[i].shadowBlur = 13
				state.arrowsInfoOpponent[i].noteShadowBlur = 13
			}
            ...
            let loop = setInterval(() => {
				...
				if (state.screenZoom < 10 && state.camZooming) {
					if (beat%4 == 0 && currentTime >= 90) state.screenZoom = 10
				...
				for (let i in state.arrowsInfo) {
					if (currentTime <= 85 && oldCurrentTime >= 76) {
						...
						if (state.arrowsInfo[i].shadowBlur < 13) state.arrowsInfo[i].shadowBlur += 0.1
						if (state.arrowsInfo[i].noteShadowBlur < 13) state.arrowsInfo[i].noteShadowBlur += 0.1
						if (state.arrowsInfoOpponent[i].shadowBlur < 13) state.arrowsInfoOpponent[i].shadowBlur += 0.1
						if (state.arrowsInfoOpponent[i].noteShadowBlur < 13) state.arrowsInfoOpponent[i].noteShadowBlur += 0.1
					}
