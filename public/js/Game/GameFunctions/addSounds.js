export default async (state) => {
    state.sounds = [
        'Sounds/scrollMenu.ogg',
        'Sounds/confirmMenu.ogg',
        'Sounds/burnSound.ogg',
        'Sounds/fnf_loss_sfx.ogg',
        'Sounds/gameOver.ogg',
        'Sounds/gameOverEnd.ogg',
        'Sounds/intro3.ogg',
        'Sounds/intro2.ogg',
        'Sounds/intro1.ogg',
        'Sounds/intro0.ogg',
        'Sounds/Meow.ogg',
        'Sounds/woeM.ogg',
        'Sounds/pop_up.ogg',
        'Sounds/alert.ogg',
        'Sounds/alertDouble.ogg',
        'Sounds/attack.ogg',
        'Sounds/attack-double.ogg',
        'Sounds/hitStatic1.ogg',
        'Sounds/simplejumpsound.ogg',
        'Sounds/datOneSound.ogg',
        'Sounds/sppok.ogg',
        'Sounds/LateNightCityTale/blackNote1.ogg',
        'Sounds/LateNightCityTale/blackNote2.ogg',
        'Sounds/LateNightCityTale/heal.ogg',
        'Sounds/LateNightCityTale/broken.ogg',
        'Sounds/LateNightCityTale/glassBreak1.ogg',
        'Sounds/LateNightCityTale/glassBreak2.ogg',

        'Musics/musics/tutorial/Inst.ogg',

        'Musics/musics/dadbattle/Inst.ogg',
        'Musics/musics/dadbattle/Voices.ogg',

        'Musics/musics/bopeebo/Inst.ogg',
        'Musics/musics/bopeebo/Voices.ogg',

        'Musics/musics/milf/Inst.ogg',
        'Musics/musics/milf/Voices.ogg',

        'Musics/musics/expurgation/Inst.ogg',
        'Musics/musics/expurgation/Voices.ogg',

        'Musics/musics/madness/Inst.ogg',
        'Musics/musics/madness/Voices.ogg',

        'Musics/musics/hellclown/Inst.ogg',
        'Musics/musics/hellclown/Voices.ogg',

        'Musics/musics/improbable-outset/Inst.ogg',
        'Musics/musics/improbable-outset/Voices.ogg',

        'Musics/musics/casanova/Inst.ogg',
        'Musics/musics/casanova/Voices.ogg',

        'Musics/musics/gospel/Inst.ogg',
        'Musics/musics/gospel/Voices.ogg',

        'Musics/musics/parish/Inst.ogg',
        'Musics/musics/parish/Voices.ogg',

        'Musics/musics/sarvente-tutorial-remix/Inst.ogg',

        'Musics/musics/worship/Inst.ogg',
        'Musics/musics/worship/Voices.ogg',

        'Musics/musics/zavodila/Inst.ogg',
        'Musics/musics/zavodila/Voices.ogg',

        'Musics/musics/unhappy/Inst.ogg',
        'Musics/musics/unhappy/Voices.ogg',

        'Musics/musics/happy/Inst.ogg',
        'Musics/musics/happy/Voices.ogg',

        'Musics/musics/really-happy/Inst.ogg',
        'Musics/musics/really-happy/Voices.ogg',

        'Musics/musics/smile/Inst.ogg',
        'Musics/musics/smile/Voices.ogg',
        
        'Musics/musics/dusk-till-dawn/Inst.ogg',
        'Musics/musics/dusk-till-dawn/Voices.ogg',

        'Musics/musics/little-man/Inst.ogg',
        'Musics/musics/little-man/Voices.ogg',

        'Musics/musics/onslaught/Inst.ogg',
        'Musics/musics/onslaught/Voices.ogg',

        'Musics/musics/ron/Inst.ogg',
        'Musics/musics/ron/Voices.ogg',

        'Musics/musics/run/Inst.ogg',
        'Musics/musics/run/Voices.ogg',

        'Musics/musics/sunshine/Inst.ogg',
        'Musics/musics/sunshine/Voices.ogg',

        'Musics/musics/trouble/Inst.ogg',
        'Musics/musics/trouble/Voices.ogg',

        'Musics/musics/withered/Inst.ogg',
        'Musics/musics/withered/Voices.ogg',

        'Musics/musics/carefree/Inst.ogg',
        'Musics/musics/carefree/Voices.ogg',

        'Musics/musics/careless/Inst.ogg',
        'Musics/musics/careless/Voices.ogg',

        'Musics/musics/censory-overload/Inst.ogg',
        'Musics/musics/censory-overload/Voices.ogg',

        'Musics/musics/cessation/Inst.ogg',
        'Musics/musics/cessation/Voices.ogg',

        'Musics/musics/interlope/Inst.ogg',
        'Musics/musics/interlope/Voices.ogg',

        'Musics/musics/termination/Inst.ogg',
        'Musics/musics/termination/Voices.ogg',

        'Musics/musics/endless/Inst.ogg',
        'Musics/musics/endless/Voices.ogg',

        'Musics/musics/too-slow/Inst.ogg',
        'Musics/musics/too-slow/Voices.ogg',

        'Musics/musics/you-cant-run/Inst.ogg',
        'Musics/musics/you-cant-run/Voices.ogg',

        'Musics/musics/triple-trouble/Inst.ogg',
        'Musics/musics/triple-trouble/Voices.ogg',

        'Musics/musics/cycles/Inst.ogg',
        'Musics/musics/cycles/Voices.ogg',

        'Musics/musics/sunshine/Inst.ogg',
        'Musics/musics/sunshine/Voices.ogg',

        'Musics/musics/chaos/Inst.ogg',
        'Musics/musics/chaos/Voices.ogg',

        'Musics/musics/faker/Inst.ogg',
        'Musics/musics/faker/Voices.ogg',

        'Musics/musics/black-sun/Inst.ogg',
        'Musics/musics/black-sun/Voices.ogg',

        'Musics/musics/milk/Inst.ogg',
        'Musics/musics/milk/Voices.ogg',

        'Musics/musics/too-fest/Inst.ogg',
        'Musics/musics/too-fest/Voices.ogg',

        'Musics/musics/execution/Inst.ogg',
        'Musics/musics/execution/Voices.ogg',

        'Musics/musics/azure-encounter/Inst.ogg',
        'Musics/musics/azure-encounter/Voices.ogg',

        'Musics/musics/secret-marilee/Inst.ogg',
        'Musics/musics/secret-marilee/Voices.ogg',

        'Musics/musics/broken-heart/Inst.ogg',
        'Musics/musics/broken-heart/Voices.ogg',

        'Musics/musics/aquaphobia/Inst.ogg',
        'Musics/musics/aquaphobia/Voices.ogg',

        'Musics/musics/lonely-sapphire/Inst.ogg',
        'Musics/musics/lonely-sapphire/Voices.ogg',
    ]

    return state.sounds.length
}